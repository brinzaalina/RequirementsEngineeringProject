...
const Filter: React.FC<FilterProps> = ({ onFilterChange }) => {
  ...
  useEffect(() => {
    // Fetch locations
    axios
      .get("http://localhost:8080/api/internships/locations")
      .then((response) => setAvailableLocations(response.data))
      .catch((error) => console.error("Error fetching locations", error));

    // Fetch fields
    axios
      .get("http://localhost:8080/api/internships/fields")
  ...
  const handleLocationChange = (
    ...
    const newFilters = {
      ...filterValues,
      locations: event.target.value as string[],
    };
  ...
  const handleFieldChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    const newFilters = {
      ...filterValues,
      fields: event.target.value as string[],
    };
    setFilterValues(newFilters);
    onFilterChange(newFilters);
  };
