...
const StudentMinimalInternshipCard: React.FC<StudentMinimalInternshipCardProps> = ({internship}) => {
    const [companyName, setCompanyName] = useState<string>('');
    const navigate = useNavigate();

    useEffect(() => {
        // Fetch the company name using the companyId from the internship
        axios.get(`http://localhost:8080/api/companies/${internship.companyId}`)
            .then(response => {
    ...
    return (
        <Card sx={{minWidth: 275, margin: 2, textAlign: 'left'}}>
            <CardContent>
                ...
                <Typography variant="body2">
                    Salary: {internship.salary > 0 ? `${internship.salary.toLocaleString()} RON` : "Unpaid"}
                </Typography>
                <Typography variant="body2">
            ...
            <CardActions>
                <Button size="small" color="primary" variant="contained" onClick={handleViewDetails}>
                    View Details
                </Button>
            </CardActions>
        </Card>
